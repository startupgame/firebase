...
import { InterstitialAdManager, AdEventType } from '../../lib/ads/adManager';
...
// Format currency utility function - memoized to improve performance
const formatCurrency = useMemo(() => {
  // Using function caching for better performance
...
// Main component - not memoized because it's the screen root
const SwipeScreen = () => {
  // Ad state management
  ...
  const initAd = async () => {
    try {
      const adInstance = await InterstitialAdManager.createAd();
  ...
  const resetTipTimer = useCallback(() => {
    setShowTip(false);
  }, []);
  ...
  const nextCard = useCallback((invested: boolean) => {
    ...
    // Clear any existing pan animation
    Animated.spring(position, {
      toValue: { x: 0, y: 0 },
      useNativeDriver: false,
    }).start();
    
    // Request an interstitial ad every few cards
    // adShownCount.current += 1;
    // if (adShownCount.current % 5 === 0) {
    //   showInterstitialAd();
    // }
    
    // Check if we need to load more cards
    if (currentIndex >= startups.length - 3 && !loading) {
  ...
  }, [currentIndex, startups, loading, currentStartup, position, page, fetchStartups, showInterstitialAd]);

// ...

  // Handle investment modal closure
  const handleInvestModalClose = useCallback(() => {
    ...
    if (!isValidAmount) {
      devLog('Invalid investment amount');
      return;
    }
    
    // After modal is closed, get ready for next startup
    setTimeout(() => {
